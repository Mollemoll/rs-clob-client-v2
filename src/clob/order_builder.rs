...
impl<K: AuthKind> OrderBuilder<Market, K> {
    ...
    pub async fn build(self) -> Result<SignableOrder> {
        ...
        // power-of-ten ticks. For a `TickSize::Other` tick (e.g. 0.02) this can yield an
        // off-grid price (0.037 -> 0.03) that the CLOB rejects; callers on such markets
        // must pass an already-grid-snapped price rather than relying on this path.
